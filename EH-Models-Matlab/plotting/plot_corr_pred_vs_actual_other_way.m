function [cc, stdcc] = plot_corr_pred_vs_actual_other_way(colours, symbols, pickedInstances, yTest, yPred, numdiff, markerSize, lineWidth, figNo, perinst)
global silentMain
...
nums = randperm(N);
len = min(N,10);
pickedInstances = nums(1:len)

%pickedparams = [2,9,12,19,22,29] % alpha=1.1,1.2,1.3; rho = 0.2,0.8
yTest = yTest(:,pickedInstances);
yPred = yPred(:,pickedInstances);
%numdiff = length(pickedparams);
